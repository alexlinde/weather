...
1. **Downsample 4x** — the full 3500x7000 grid is reduced to 875x1750 via block averaging to keep transient memory bounded (~6 MB per array vs ~98 MB at full res). This matters most during the startup seed, when many such arrays are allocated concurrently across 8 worker threads (see CLOUD.md → Memory & Swap).

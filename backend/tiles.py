...
def _compute_atlas_array(
    ...
    """Build the 2048×256 uint8 atlas array (8 stacked 256×256 tilt bands).

    Returns ``None`` when the tile is entirely outside the grid or has no echo,
    so callers can short-circuit to a shared empty result.
    """
...
def render_atlas_tile(
    ...
    Uses Mercator-correct per-pixel sampling so adjacent tiles produce identical
    values at their shared boundary. Returns PNG bytes.
    """
    atlas = _compute_atlas_array(sparse_grids, metadata, z, x, y)
    if atlas is None:
...
_EMPTY_RAW: bytes | None = None


def _get_empty_raw() -> bytes:
    """Cached all-zero raw atlas buffer (256×2048 bytes, single channel)."""
    global _EMPTY_RAW
    if _EMPTY_RAW is None:
        _EMPTY_RAW = bytes(ATLAS_BAND_SIZE * ATLAS_BAND_SIZE * ATLAS_NUM_BANDS)
    return _EMPTY_RAW


def render_atlas_raw(
    sparse_grids: dict[str, sp.csr_matrix],
    metadata: dict,
    z: int,
    x: int,
    ...
    """Render an atlas tile as raw single-channel uint8 bytes (256×2048).

    Row-major: 8 vertically-stacked 256×256 bands. Intended for direct upload to
    a GPU ``DataTexture`` on platforms without a PNG/canvas decode path (native
    React Native). Always returns exactly 256*2048 bytes.
    """
    atlas = _compute_atlas_array(sparse_grids, metadata, z, x, y)
    if atlas is None:
        return _get_empty_raw()
    return atlas.tobytes()


atlas_tile_cache = TileCache(max_size=_TILE_CACHE_MAX)
raw_tile_cache = TileCache(max_size=_TILE_CACHE_MAX)

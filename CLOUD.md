...
```
┌─────────────────────────────────────────────────────────┐
│  GCE VM: weather-radar (e2-standard-2, us-east4-c)     │
...
| OS | Debian 12 |
| Boot Disk | 20 GB |
| Swap | 4 GB swapfile (`/swapfile`, persisted in `/etc/fstab`) |
...
The app's startup seed is the memory high-water mark. Seeding 60 frames builds the sparse `ConusTiltCache` (~1.2 GB for 30 frames) while also pre-rendering atlas tiles, computing motion fields, and running virtual-volume advection — each of which allocates transient dense 3500×7000 arrays (~98 MB each) across 8 worker threads.

On the original `e2-medium` (4 GB, no swap) this peaked at ~3.4 GB RSS and the kernel OOM-killer repeatedly killed `uvicorn` (exit 137) mid-seed, leaving Caddy up but the backend dead in a restart loop. Two changes fixed this:

1. **Upgraded to `e2-standard-2` (8 GB RAM).** Post-seed steady state is ~2.4 GB used, leaving ~5 GB headroom.
2. **Added a 4 GB swapfile** as a safety net (`/swapfile`, persisted in `/etc/fstab`). The VM ships with zero swap by default, so any spike above physical RAM was an instant kill with nothing to fall back on.

To recreate the swapfile (e.g. on a fresh VM):
...
Diagnosing an OOM kill: `sudo dmesg -T | grep -i "killed process"` shows the kernel OOM events; `sudo docker inspect weather-radar-app-1 --format '{{.State.OOMKilled}} {{.RestartCount}}'` confirms the container was OOM-killed.

> **Notes:**
> - `deploy.sh` defaults `MACHINE_TYPE="e2-standard-2"`, but that value is only consumed by `cmd_setup` when first creating the VM. For an existing VM the machine type is changed in place via `gcloud compute instances set-machine-type` (stop → set-machine-type → start) and is not managed by `deploy.sh`.
> - `setup` does **not** create the swapfile — run the swap commands above manually on a fresh VM.

## Cost

The `e2-standard-2` VM in `us-east4-c` costs approximately $50/month when running continuously (up from ~$25 for the former `e2-medium`). Use `./deploy.sh stop` to stop the VM when not needed (no compute charges while stopped; the static IP incurs a small charge of ~$7/month when not attached to a running VM).
...
**Site loads slowly/then stops, or `status` shows only the caddy container:**
The app container was likely OOM-killed and is down (Caddy stays up, so the site returns 502 with no backend). Confirm with `sudo docker inspect weather-radar-app-1 --format '{{.State.OOMKilled}} {{.RestartCount}}'` and `sudo dmesg -T | grep -i "killed process"`. See the **Memory & Swap** section. With the `e2-standard-2` + swapfile this should no longer happen; if it recurs, the seed's transient allocation has grown and the VM may need more RAM or reduced seed concurrency.
